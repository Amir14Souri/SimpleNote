...
import ir.sharif.simplenote.data.remote.model.ErrorCode404Enum
...
data class Error404 (
    ...
    class CustomTypeAdapterFactory : TypeAdapterFactory {
        override fun <T> create(gson: Gson, type: TypeToken<T>): TypeAdapter<T>? {
            if (!Error404::class.java.isAssignableFrom(type.rawType)) {
              return null // this class only serializes 'Error404' and its subtypes
            }
            val elementAdapter = gson.getAdapter(JsonElement::class.java)
            val thisAdapter = gson.getDelegateAdapter(this, TypeToken.get(Error404::class.java))

            @Suppress("UNCHECKED_CAST")
            return object : TypeAdapter<Error404>() {
                @Throws(IOException::class)
                override fun write(out: JsonWriter, value: Error404) {
                    val obj = thisAdapter.toJsonTree(value).getAsJsonObject()
                    elementAdapter.write(out, obj)
                }

                @Throws(IOException::class)
                override fun read(jsonReader: JsonReader): Error404  {
    ...
    */
    ...
    fun validateJsonElement(jsonElement: JsonElement?) {
      if (jsonElement == null) {
        throw IOException("The required field(s) code,detail,attr in Error404 is not found in the empty JSON string")
      }
      ...
      require(ErrorCode404Enum.values().any { it.value == jsonObj["code"].asString }) {
        String.format("Expected the field `code` to be valid `ErrorCode404Enum` enum value in the JSON string but got `%s`", jsonObj["code"].toString())
      }
