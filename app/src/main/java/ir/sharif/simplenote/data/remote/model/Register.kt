...
data class Register (
    ...
    class CustomTypeAdapterFactory : TypeAdapterFactory {
        override fun <T> create(gson: Gson, type: TypeToken<T>): TypeAdapter<T>? {
            if (!Register::class.java.isAssignableFrom(type.rawType)) {
              return null // this class only serializes 'Register' and its subtypes
            }
            val elementAdapter = gson.getAdapter(JsonElement::class.java)
            val thisAdapter = gson.getDelegateAdapter(this, TypeToken.get(Register::class.java))

            @Suppress("UNCHECKED_CAST")
            return object : TypeAdapter<Register>() {
                @Throws(IOException::class)
                override fun write(out: JsonWriter, value: Register) {
                    val obj = thisAdapter.toJsonTree(value).getAsJsonObject()
                    elementAdapter.write(out, obj)
                }

                @Throws(IOException::class)
                override fun read(jsonReader: JsonReader): Register  {
    ...
    */
    ...
    fun validateJsonElement(jsonElement: JsonElement?) {
      if (jsonElement == null) {
        throw IOException("The required field(s) username,email in Register is not found in the empty JSON string")
      }
