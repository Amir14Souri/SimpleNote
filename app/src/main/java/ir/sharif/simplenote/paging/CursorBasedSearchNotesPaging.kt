...
class CursorBasedSearchNotesPagingSource(
    private val query: String,
    private val queries: NotesQueries,
) : PagingSource<CursorBasedSearchNotesPagingSource.CursorKey, Notes>() {
            ...
            withContext(Dispatchers.IO) {
                ...
                val notes = when (params) {
                    is LoadParams.Refresh -> {
                        queries.searchNotesInital(query, pageSize.toLong()).executeAsList()
                    }
                    is LoadParams.Append -> {
                        val cursor = params.key
                        queries.searchNotesAfterCursor(
                            query,
                            cursor.createdAt,
                            cursor.id,
                            pageSize.toLong()
